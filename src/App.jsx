...
// Lazy Components - ตรวจสอบชื่อไฟล์ในเครื่องด้วยว่าสะกดตรงเป๊ะไหม
...
function App() {
  ...
  // ฟังก์ชันจัดการ Class ของ Navbar
  const getNavClass = (path) => {
    // เช็คว่า path ปัจจุบันตรงกับเมนูไหม (รองรับทั้ง / และ /shop)
    ...
    <>
      ...
      <div className="min-h-screen py-6 relative flex flex-col bg-[#0b0a10]">
        ...
        <nav className="sticky top-0 z-50 w-full mb-8 pt-4">
          <div className="max-w-4xl mx-auto w-full px-4">
            <div className="bg-white/[0.05] backdrop-blur-xl border border-white/10 rounded-full flex justify-between items-center px-6 py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
              <div className="flex items-center space-x-2 md:space-x-6 text-sm font-sarabun">
                <Link to="/" className={getNavClass('/')}>
                  {t('home')}
                </Link>
                {/* ใช้ Anchor Tag สำหรับ ID Link ในหน้าเดียวกัน */}
        ...
        <main className="flex-1 w-full max-w-6xl mx-auto px-2 md:px-4">
          <Suspense fallback={<div className="text-white text-center mt-20">Loading Destiny...</div>}>
            <Routes>
              <Route path="/" element={<Home />} />
              <Route path="/shop" element={<Shop />} />
              {/* Fallback สำหรับหน้าไม่พบ (404) */}
              <Route path="*" element={<Home />} />
            </Routes>
          </Suspense>
        </main>
        ...
        {/* 🌙 Footer */}
        <footer className="mt-20 text-center text-gray-500 text-sm pb-6 z-10 relative">
